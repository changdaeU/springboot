...
public class ValiController {
	...
	public String create( @ModelAttribute("dto") ContentDto contentdto, Model model,
		...
		// 멤버변수에 자동 대입됩니다. 
		// wirter -> contentdto.writer     content -> contentdto.content 
		
		// 추가로 contentdto 라는 이름으로 객체가 model 객체게 추가되서 최종 jsp 파일에 전달됩니다
		
		// 맴개변수 객체(매개변수) 앞에 ModelAttribute(전달이름)을 붙이면,
		// return 되는 페이지에 해당 객체가 . model.addAttribute  로 넣은것처럼
		// 같이 전달됩니다. model.addAttribute("dto" , contendto );   와 동일한 동작
		...
		// 리턴되지 않아도 call by reference 이기 때문에 오류 내용을 현재 위치에서도 result라는 이름으로
		// 사용이 가능합니다.
		
		// 전달된 파라미터가 비어있더나 널이라면, 그 값을 갖고 원래의 페이지로 돌아가고, 
		// 유효한 데이터라면 DonePage.jsp 로 전송됩니다
		/*if( true ) {
			model.addAttribute("message", "writer 또는  contrent 를 입력하세요");
			return "startPage";
		}*/
		if(result.hasErrors()) {
